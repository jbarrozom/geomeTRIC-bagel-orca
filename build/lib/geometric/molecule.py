...
class Molecule(object):
    ...
    def read_dcd(self, fnm, **kwargs): # pragma: no cover
        ...
        Answer = {'xyzs' : xyzs,
                  'boxes' : boxes}
        return Answer
    def read_bagel(self, fnm, **kwargs):
        elem    = []
        xyz     = []
        ln      = 0
        absln   = 0
        ...
        inxyz = 0
        ## read json
        with open(fnm, 'r') as fcc_file:
             fcc_data = json.load(fcc_file)
        aa=(fcc_data['bagel'][0])
        bb=(aa['geometry'])
        ## atomos en una lista de dictionaries
        for ibongo in range(len(bb)):
            atoma=bb[ibongo]
            geo=atoma['xyz']
            xx=geo[0]
            yy=geo[1]
            zz=geo[2]
            ## considerer that the input is writeen in Bohrs , convert to angstroms
            ## 1 bohr = 0.529177249 angstrom
            xx2=float(xx)*float(0.529177249)
            yy2=float(yy)*float(0.529177249)
            zz2=float(zz)*float(0.529177249)
            sim=atoma['atom']
            sim2=sim[0]
            print(ibongo,atoma,sim,geo)
            elem.append(sim)
            xyz.append(np.array([float(xx2), float(yy2), float(zz2)]))
            
        #xyz.append(np.array([float(xx), float(yy), float(zz)]))
        # cabellos carga  
        charge=0
        mult=1
        #if there is a file:
        if os.path.exists('charge.multiplicity.dat'):
             print('Warrning : charge and mult are reading from file: charge.multiplicity.dat')
             print('Warrning : charge and mult are reading from file: charge.multiplicity.dat')
             bifile=open('charge.multiplicity.dat',"r")
             for line3 in bifile:
                 line3=line3.strip(' \t\n\r')
                 if len(line3.strip()) != 0 :
                    litio = line3.lstrip()
                    if not litio.startswith("#"):
                        readline=line3.split()
                        if len(readline) == 2:
                            if (line3.split()[0]).lower() == "charge": 
                               charge=(line3.split()[1])
                               print('Charge is reading from file: charge.multiplicity.dat')
                               print('Charge =',charge)
                            if (line3.split()[0]).lower() == "multiplicity": 
                               mult=(line3.split()[1])
                               print('Multi is reading from file:  charge.multiplicity.dat')
                               print('Mult   =',mult)
             bifile.close()
    ...
    def read_orca(self, fnm, **kwargs):
        ...
        ### esta carga cambiarla accorde a la necitada ## cabellos 
        charge=0
        mult=1
